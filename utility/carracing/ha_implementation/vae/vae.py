...
class ConvVAE(object):
  ...
  def _build_graph(self):
    ...
    with self.g.as_default():
      ...
      if self.is_training:
        ...
        self.optimizer = tf.train_or_reload_models.AdamOptimizer(self.lr)
  ...
  def save_model(self, model_save_path):
    sess = self.sess
    with self.g.as_default():
      saver = tf.train_or_reload_models.Saver(tf.global_variables())
  ...
  def load_checkpoint(self, checkpoint_path):
    ...
    with self.g.as_default():
      saver = tf.train_or_reload_models.Saver(tf.global_variables())
    ckpt = tf.train_or_reload_models.get_checkpoint_state(checkpoint_path)
