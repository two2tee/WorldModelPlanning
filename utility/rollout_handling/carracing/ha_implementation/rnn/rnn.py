...
def default_hps():
  return HyperParams(num_steps=2000, # train model for 2000 steps.
                     ...
                     input_seq_width=35,    # width of our data_random_car (32 + 3 actions)
                     output_seq_width=32,    # width of our data_random_car is 32
...
class MDNRNN():
  ...
  def build_model(self, hps):
    ...
    cell_fn = tf.nn.rnn_cell.LSTMCell  #tf.nn.rnn_cell.LSTMCell #tf.contrib.rnn.LayerNormBasicLSTMCell # use LayerNormLSTM
    ...
    # reshape target data_random_car so that it is compatible with prediction shape
    ...
    if self.hps.is_training == 1:
      ...
      optimizer = tf.train_or_reload_models.AdamOptimizer(self.lr)
...
def sample_sequence(sess, s_model, hps, init_z, actions, temperature=1.0, seq_len=1000):
  ...
  if prev_data is not None:
    # encode the previous data_random_car into the hidden state first
