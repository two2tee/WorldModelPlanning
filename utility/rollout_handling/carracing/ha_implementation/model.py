...
class Model:
  ...
  def __init__(self, load_model=True):
    ...
    if load_model:
      ...
      self.vae.load_json(jsonfile=f"{os.getcwd()}/utility/carracing/ha_implementation/vae/vae.json")
      self.rnn.load_json(jsonfile=f"{os.getcwd()}/utility/carracing/ha_implementation/rnn/rnn.json")
  ...
  def load_model(self, filename):
    ...
    self.data = data
    model_params = np.array(data[0])  # assuming other stuff is in data_random_car
